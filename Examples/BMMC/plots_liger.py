...
clusts = [8, 20, 30]
for clust in clusts:
    path = "./Liger_small/k_" + str(clust) + "/"
    data_dir = "../../data/real/diag/BMMC/"
    ...
    for batch in range(0,2):
        labels.append(pd.read_csv(os.path.join(data_dir, 'meta_c' + str(batch + 1) + '.csv'), index_col=0)["cluster"].values.squeeze())
