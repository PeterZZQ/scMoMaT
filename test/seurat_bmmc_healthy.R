...
path <- "../data/real/diag/healthy_hema/topgenes_2000/BMMC/"
result_path <- "bmmc_healthyhema_2000/seurat/"
...
seurat.atac <- CreateSeuratObject(
  ...
  assay = 'peaks',
  project = 'ATAC',
  meta.data = read.table(paste0(path, "meta_c1.csv"), header = T, row.names = 1, sep = ",")
)
...
seurat.rna <- CreateSeuratObject(counts = counts.rna, 
                                 assay = "RNA", 
                                 project = "full_matrix", 
                                 meta.data = read.table(paste0(path, "meta_c2.csv"), header = T, row.names = 1, sep = ",")
)
...
# Plot the data
p1 <- DimPlot(seurat.rna, group.by = "BioClassification", label = TRUE) + NoLegend() + ggtitle("RNA")
p2 <- DimPlot(seurat.atac, group.by = "BioClassification", label = TRUE) + NoLegend() + ggtitle("ATAC")
p1 + p2
...
predicted.labels <- TransferData(
  ...
  refdata = seurat.rna@meta.data$BioClassification,
  weight.reduction = seurat.atac[['lsi']],
  dims = 2:30
)
seurat.atac <- AddMetaData(object = seurat.atac, metadata = predicted.labels)
# Plot the result
plot1 <- DimPlot(seurat.atac, group.by = 'BioClassification', label = TRUE, repel = TRUE) + NoLegend() + ggtitle('Ground truth annotation')
...
coembed <- merge(x = seurat.atac, y = seurat.rna)
...
# Plot the dataset
DimPlot(coembed, group.by = c("orig.ident", "BioClassification"))

write.table(coembed@reductions$pca@cell.embeddings[1:dim(seurat.atac)[2], ], file = paste0(result_path, "seurat_pca_c1.txt"), sep = "\t")
write.table(coembed@reductions$pca@cell.embeddings[(dim(seurat.atac)[2]+1):(dim(seurat.atac)[2] + dim(seurat.rna)[2]), ], file = paste0(result_path, "seurat_pca_c2.txt"), sep = "\t")
write.table(coembed@reductions$umap@cell.embeddings[1:dim(seurat.atac)[2], ], file = paste0(result_path, "seurat_umap_c1.txt"), sep = "\t")
write.table(coembed@reductions$umap@cell.embeddings[(dim(seurat.atac)[2]+1):(dim(seurat.atac)[2] + dim(seurat.rna)[2]), ], file = paste0(result_path, "seurat_umap_c2.txt"), sep = "\t")
