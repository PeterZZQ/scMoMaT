...
result_dir <- "../bmmc_healthyhema_1000/stabmap/"

RxC1 <- readMM(paste0(data_dir, "RxC1.mtx"))
rownames(RxC1) <- read.table(paste0(data_dir, "regions.txt"), header = F, sep = ",")[[1]]
colnames(RxC1) <- rownames(read.table(paste0(data_dir, "meta_c1.csv"), header = T, row.names = 1, sep = ","))

GxC2 <- readMM(paste0(data_dir, "GxC2.mtx"))
rownames(GxC2) <- read.table(paste0(data_dir, "genes.txt"), header = F, sep = ",")[[1]]
colnames(GxC2) <- rownames(read.table(paste0(data_dir, "meta_c2.csv"), header = T, row.names = 1, sep = ","))
