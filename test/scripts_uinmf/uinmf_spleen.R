...
counts.atac <- readMM(paste0(data_dir, "RxC2.mtx"))
rownames(counts.atac) <- read.table(paste0(data_dir, "regions.txt"), header = F, sep = ",")[[1]]
colnames(counts.atac) <- rownames(read.csv(paste0(data_dir, "meta_c2.csv"), header = T, row.names = 1, sep = ","))
meta.atac <- read.csv(paste0(data_dir, "meta_c2.csv"), header = T, row.names = 1, sep = ",")

counts.rna <- readMM(paste0(data_dir, "GxC1.mtx"))
rownames(counts.rna) <- read.table(paste0(data_dir, "genes.txt"), header = F, sep = ",")[[1]]
colnames(counts.rna) <- rownames(read.csv(paste0(data_dir, "meta_c1.csv"), header = T, row.names = 1, sep = ","))
meta.rna <- read.csv(paste0(data_dir, "meta_c1.csv"), header = T, row.names = 1, sep = ",")
...
counts.bin <- readMM(paste0(data_dir, 'BxC2.mtx'))
...
#
################################################
result_dir <- "../spleen/liger/"
result_dir <- "../spleen/remove_celltype/liger/"
...
#
################################################
result_dir <- "../spleen/uinmf_bin/"
result_dir <- "../spleen/remove_celltype/uinmf_bin/"
