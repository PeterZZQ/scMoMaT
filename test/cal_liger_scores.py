...
# In[4] 
# For quantile on CFRM
# CFRM diag
runs = 5

dir1 = '../data/simulated/'
dir2s = ["./results_multi/diag/", "./results_multi/hori/", "./results_multi/multi3/", "./results_multi/multi4/"]
    ...
    for path in paths:
        ...
        for i in range(3):
            ...
            for run in range(runs):
                pred_label = [x for x in pd.read_csv(os.path.join(dir2 + path[:-1] + "_" + str(i) + "_" + str(run) + "_clust_id.csv"), index_col=0, sep = ",")["x"].values]
                pred_label = np.array(pred_label)
                ari.append(adjusted_rand_score(labels_pred = pred_label, labels_true = np.concatenate((label_b1, label_b2), axis = 0)))

            ari = np.array(ari)
            np.save(file = dir2 + path[:-1] + "_" + str(i) + "_ari_cfrm_quantile.npy", arr = ari)
