...
paths = ['2b4c_sigma0.1_b2_1/', '2b4c_sigma0.1_b2_2/', '2b4c_sigma0.2_b2_1/', '2b4c_sigma0.2_b2_2/', '2b4c_sigma0.3_b2_1/', '2b4c_sigma0.3_b2_2/']
paths = ['2b4c_sigma0.1_b1_1/', '2b4c_sigma0.1_b1_2/', '2b4c_sigma0.2_b1_1/', '2b4c_sigma0.2_b1_2/', '2b4c_sigma0.3_b1_1/', '2b4c_sigma0.3_b1_2/']
paths = ['2b4c_sigma0.4_b1_1/', '2b4c_sigma0.4_b1_2/']
...
for path in paths:
    ...
    for i, alpha in enumerate(alphas):
        ...
        for run in range(runs):
            ...
            print("CFRM...")
            # our model
            model1 = model.cfrm(counts1, Ns = Ns, K = K, N_feat = N_feat, batch_size = batchsize, interval=1000, lr=1e-3, alpha =alpha, seed = run).to(device)
            ...
            np.save("./results_multi/vert/" + path[:-1] + "_" + str(i) + "_" + str(run) + "_C0.npy", model1.Cs[0].cpu().detach().numpy())
            ...
            z = umap_op.fit_transform(z_b1)

            utils.plot_latent(z, z, label_b1, label_b1, mode= "separate", save = "./results_multi/vert/plots/" + path[:-1] + "_" + str(run) + "_" + str(i) + "_cfrm.png")
            
            utils.plot_latent(z, z, max_b1, max_b1, mode= "separate", save = "./results_multi/vert/plots/predict_" + path[:-1] + "_" + str(run) + "_" + str(i) + "_cfrm.png")
...
#             g.savefig("./results/plots/clustmap_atac_" + path[:-1] + "_" + str(run) + "_" + str(i) + "_cfrm.png")
            ...
            ari_cfrm.append(adjusted_rand_score(labels_pred = max_b1, labels_true = label_b1))
        
        np.save( "./results_multi/vert/" + path[:-1] + "_" + str(i) + "_ari_cfrm.npy", np.array(ari_cfrm)) 
        np.save("./results_multi/vert/" + path[:-1] + "_" + str(i) + "_losses.npy", np.array(losses))
