    ...
    for latent_dim in [3,5,7,10]:
        ...
        for batchsize in [0.1, 0.5, 1]:
            ...
            for run in range(3):
                ...
                # train model
                model1 = newmodel.cfrm_new(counts, N = latent_dim, batch_size = batchsize, interval=5000, lr=1e-3, 
                                           alpha = [1000, 1000, 100, 100], seed = run).to(device)
                ...
                if loss < best_loss:
                    torch.save(model1.state_dict(), "./results/models/" + path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_best.pt")
...
#                 z_rna, z_atac = match_alignment(z_rna, z_atac, k = 10)
...
#                 max_atac = np.argmax(z_atac, axis = 1).squeeze()
                ...
                utils.plot_latent(z[:z_rna.shape[0],:], z[z_rna.shape[0]:,:], label_rna, label_atac, mode= "separate", save = "./results/plots/" + path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_" + str(run) + ".png")
                ...
                np.save( "./results/" + path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_" + str(run) + "_bm_enropy.npy", np.array(bm_entro))
                fig = plt.figure(figsize = (5,5))
                ax = fig.add_subplot()
                sns.boxplot(x = pd.DataFrame(data = bm_entro[:,None], columns = ["bm_entro"])["bm_entro"], ax = ax)
                fig.savefig("./results/"+ path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_" + str(run) + "_bm_enropy.png")
            ...
            np.save( "./results/" + path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_ari.npy", np.array(ari_scores))
            np.save( "./results/" + path[:-1] + "_" + str(latent_dim) + "_" + str(batchsize) + "_loss.npy", np.array(losses))
        ...
        # save kl divergence
        np.save("./results/" + path[:-1] + "_" + str(latent_dim) + "_kl.npy", kl)
