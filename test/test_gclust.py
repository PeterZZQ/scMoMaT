...
# paths = ['gene_sigma_0.2_1/', 'gene_sigma_0.2_2/']
...
for path in paths:
    ...
    counts6 = {"rna":[counts_rna1], "atac":[None], "gact":A}
    ...
    for i, alpha in enumerate(alphas):
        ...
        for run in range(runs):
            print("data: "+ path[:-1] + "\nrun: "+ str(run))
            print("CFRM...")
            model1 = model.cfrm(counts6, Ns = Ns[0:1], K = K, N_feat = N_feat, batch_size = batchsize, interval=1000, lr=1e-3, alpha =alpha, seed = run).to(device)
            ...
            np.save("./results_gclust/single/" + path[:-1] + "_" + str(i) + "_" + str(run) + "_C1.npy", model1.Cs[0].cpu().detach().numpy())
            np.save("./results_gclust/single/" + path[:-1] + "_" + str(i) + "_" + str(run) + "_Ag.npy", model1.Ag.cpu().detach().numpy())
            np.save("./results_gclust/single/" + path[:-1] + "_" + str(i) + "_" + str(run) + "_Cg.npy", model1.Cg.cpu().detach().numpy())
            ...
            utils.plot_latent(z_g, z_g, label_g, max_g, mode= "separate", save = "./results_gclust/single/plots/" + path[:-1] + "_" + str(run) + "_" + str(i) + "_cfrm.png")            
        ...
        np.save( "./results_gclust/single/" + path[:-1] + "_" + str(i) + "_ari_gene.npy", np.array(ari_gene)) 
        np.save( "./results_gclust/single/" + path[:-1] + "_" + str(i) + "_losses.npy", np.array(losses))
