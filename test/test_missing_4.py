    ...
    for missing in range(2):
        ...
        for i, alpha in enumerate(alphas):
            ...
            for run in range(runs):
                print("CFRM...")
                # our model
                model1 = diffmodel.cfrm_diff(counts, N1 = N1, N2 = N2, K = K, batch_size = batchsize, interval = 1000, lr = lr, 
                                                alpha = alpha, seed = run, learn_gact = False).to(device)

                model1.train_func(T = 6000)
                ...
                g.savefig("./results_missing/plots/clustmap_rna_" + path[:-1] + "_" + str(missing) + "_" + str(i) + "_" + str(run) + "_cfrm.png")

                g = sns.clustermap(z_atac, figsize = (7,7))
                g.savefig("./results_missing/plots/clustmap_atac_" + path[:-1] + "_" + str(missing) + "_" + str(i) + "_" + str(run) + "_cfrm.png")
