...
for batch in range(n_batches):        
    ...
    try:
        counts_protein = np.loadtxt(os.path.join(dir, 'PxC' + str(batch + 1) + ".txt"), delimiter = "\t").T
        ...
        # utils.plot_latent_ext([x_umap], annos = [labels[-1]], mode = "joint", save = scmomat_dir + f'PxC{batch+1}', figsize = (10,7), axis_label = "UMAP", markerscale = 6, s = 5, label_inplace = True)
...
if result_dir[-1] == "1":
    # protein is the shared modalitiy across batches, add pseudo-protein counts
    counts["rna"][0] = None
    counts["rna"][1] = None
    counts["rna"][2] = None
    counts["rna"][3] = None
    ...
    counts["atac"][4] = None
    counts["atac"][5] = None
    counts["protein"][0] = None
    counts["protein"][1] = None
    counts["protein"][4] = None
    counts["protein"][5] = None
if result_dir[-1] == "2":
    # no shared modalitiy across batches, should still work for simulated datasets without pseudo-counts
    counts["rna"][0] = None
    counts["rna"][1] = None
    counts["rna"][2] = None
    counts["atac"][3] = None 
...
RxG = np.loadtxt(os.path.join(dir, 'region2gene.txt'), delimiter = "\t")
PxG = np.loadtxt(os.path.join(dir, 'protein2gene.txt'), delimiter = "\t")
RxP = RxG @ PxG.T
RxP = (RxP > 0) 
...
if USE_PSEUDO == True:
    ...
    for idx in range(len(counts["atac"])):
        if (counts["protein"][idx] is None) & (counts["atac"][idx] is not None):
            ...
            counts["protein"][idx] = counts["atac"][idx] @ RxP
            #BINARIZE, still is able to see the cluster pattern, much denser than scRNA-Seq (cluster pattern clearer)
            counts["protein"][idx] = (counts["protein"][idx]!=0).astype(int)

        if (counts["protein"][idx] is None) & (counts["rna"][idx] is not None):
            print(f"pseudo-protein for rna in batch {idx}")
            counts["protein"][idx] = counts["rna"][idx] @ PxG.T
            # pseudo-protein counts = gene expression count for only protein related genes, if binarize, could be all non-zero
            # counts["protein"][idx] = (counts["protein"][idx]!=0).astype(int)
...
proteins = np.array(["protein_" + str(x) for x in range(counts["protein"][2].shape[1])])
...
torch.save(model1, scmomat_dir + f'scMoMaT_{K}_{T}.pt')
model1 = torch.load(scmomat_dir + f'scMoMaT_{K}_{T}.pt')
...
if False:
    ...
    for seed in [1,2,3,4,5,6,7,9]:
        result_dir = f'simulated/6b16c_test_{seed}/protein_scenario1/'
    ...
    fig.tight_layout()
    fig.savefig("simulated/scores_protein_scenario1.png", bbox_inches = "tight")
