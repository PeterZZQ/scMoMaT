...
start_time = time.time()
model_ext = model.cfrm_vanilla(counts = counts, K = K, batch_size = batchsize, interval = interval, lr = lr, alpha = alpha, seed = run).to(device)
losses1 = model_ext.train_func(T = T)
end_time = time.time()
print("running time: " + str(end_time - start_time))
...
for mod in model_ext.A_assos.keys():
    if mod != "shared":
        print("minimum")
        print(mod)
        print(torch.min(model_ext.A_assos["shared"] + model_ext.A_assos[mod]).item())

for mod in model_ext.A_assos.keys():
    if mod != "shared":
        print("mean")
        print(mod)
        print(torch.mean(model_ext.A_assos["shared"] + model_ext.A_assos[mod]).item())

for mod in model_ext.A_assos.keys():
    if mod != "shared":
        print("maximum")
        print(mod)
        print(torch.max(model_ext.A_assos["shared"] + model_ext.A_assos[mod]).item())

print(model_ext.scales)

umap_op = UMAP(n_components = 2, n_neighbors = 15, min_dist = 0.4, random_state = 0) 
zs = []
for batch in range(0,n_batches):
    z = model_ext.softmax(model_ext.C_cells[str(batch)].cpu().detach()).numpy()
    zs.append(z)
