...
#         self.A_12 = torch.ones(N, 1)
#         self.W = Parameter(torch.eye(N))
#
#         self.optimizer = opt.Adam(self.parameters(), lr=lr)
#
#     def entropy_loss(self, C):
#         z = softmax(C, dim=1) @ self.W
#         print(z.shape)
#         loss = - softmax(z, dim=1) * log_softmax(z, dim=1)
#         print(loss.sum(dim=1).mean().item())
#         return loss.sum(dim=1).mean()
#
#     def batch_loss(self, mode='C_c'):
#         if mode == 'C_c':
#             loss1 = (self.G - softmax(self.C_1, dim=1) @ self.A_1g.detach() @ self.C_g.detach().t()).pow(2).mean()
#             loss2 = (self.R - softmax(self.C_2, dim=1) @ self.A_2r.detach() @ self.C_r.detach().t()).pow(2).mean()
#             loss5 = sum(map(self.entropy_loss, [self.C_1, self.C_2]))
#             loss3, loss4 = 0, 0
#         elif mode == 'C_r':
#             loss2 = (self.R - softmax(self.C_2.detach(), dim=1) @ self.A_2r.detach() @ self.C_r.t()).pow(2).mean()
#             loss3 = (self.A - self.C_r @ self.C_g.detach().t()).pow(2).mean()
#             loss1, loss4, loss5 = 0, 0, 0
#         elif mode == 'C_g':
#             loss1 = (self.G - softmax(self.C_1.detach(), dim=1) @ self.A_1g.detach() @ self.C_g.t()).pow(2).mean()
#             loss3 = (self.A - self.C_r.detach() @ self.C_g.t()).pow(2).mean()
#             loss2, loss4, loss5 = 0, 0, 0
#         elif mode == 'A':
#             loss1 = (self.G - softmax(self.C_1.detach(), dim=1) @ self.A_1g @ self.C_g.detach().t()).pow(2).mean()
#             loss2 = (self.R - softmax(self.C_2.detach(), dim=1) @ self.A_2r @ self.C_r.detach().t()).pow(2).mean()
#             loss3, loss4, loss5 = 0, 0, 0
#         elif mode == 'valid':
#             with torch.no_grad():
#                 loss1 = (self.G - softmax(self.C_1, dim=1) @ self.A_1g @ self.C_g.t()).pow(2).mean()
#                 loss2 = (self.R - softmax(self.C_2, dim=1) @ self.A_2r @ self.C_r.t()).pow(2).mean()
#                 loss3 = (self.A - self.C_r @ self.C_g.t()).pow(2).mean()
...
#         count = 0
#         for t in range(T):
#             for mode in ['C_c', 'C_r', 'C_g', 'A']:
#                 self.optimizer.zero_grad()
#                 loss, *_ = self.batch_loss(mode)
#                 loss.backward()
#                 self.optimizer.step()
#             self.D_12 = ((self.A_1g * self.A_2r).sum(dim=1) / self.A_2r.pow(2).sum(dim=1)).detach()
...
#                 'loss gene act: {:.5f}'.format(loss3.item()),
#                 'loss merge: {:.5f}'.format(loss4.item()),
#                 'loss entropy: {:.5f}'.format(loss5.item())
#             ]
#             for i in info:
#                 print("\t", i)
#             if loss.item() < best_loss:
#                 best_loss = loss.item()
#                 torch.save(self.state_dict(), f'../check_points/simulated_2b_{self.N}c.pt')
#                 count = 0
#             else:
#                 count += 1
#                 if count % 20 == 0:
#                     self.load_state_dict(torch.load(f'../check_points/simulated_2b_{self.N}c.pt'))
#                     self.optimizer.param_groups[0]['lr'] *= 0.5
#                     print('Epoch: {}, shrink lr to {:.4f}'.format(t + 1, self.optimizer.param_groups[0]['lr']))
#                     if self.optimizer.param_groups[0]['lr'] < 1e-4:
#                         break
