...
class NMF(Module):
    ...
    """
    def __init__(self, nfeatures1, nfeatures2, N):
        super().__init__()
        # projection
        self.P1 = Parameter(torch.rand(nfeatures1, N))
        self.P2 = Parameter(torch.rand(nfeatures2, N))
        
        # interaction
        self.Ai = Parameter(torch.rand(N, N))
        
        # one entity
        # self.W1 = Parameter(torch.rand(N,1))
        # self.W2 = Parameter(torch.rand(1,N))
        
        self.W1 = torch.zeros((N,1)).to(device)
        self.W2 = torch.zeros((1,N)).to(device)
    ...
    def forward(self, u, v):
        C_u = self.act(u @ self.P1)
        C_v = self.act(v @ self.P2)
        return self.act(C_u @ self.Ai @ C_v.t() + C_u @ self.W1 + self.W2 @ C_v.t(), mode = "sigmoid")

class cfrm_deep(Module):
    def __init__(self, counts, N=3, batch_size=1.0, interval=10, lr=1e-3, alpha = [1000, 1000, 100, 0.0, 0.0]):
        super().__init__()
        self.N = N
        ...
        self.nmf_rna = NMF(nfeatures1 = self.G.shape[0], nfeatures2 = self.G.shape[1], N = self.N)
        self.nmf_atac = NMF(nfeatures1 = self.R.shape[0], nfeatures2 = self.R.shape[1], N = self.N)
...
#             print(torch.sum(ind_r[:, mask_r] - torch.eye(mask_r.shape[0]).cuda()))
            ...
            G_est = self.nmf_rna(u = ind_1, v = ind_g)
            R_est = self.nmf_atac(u = ind_2, v = ind_r)
            
            loss_rna = self.recon_loss(x_est = G_est, x = self.G[mask_1,:][:, mask_g], mode = "RNA") 
            loss_atac = self.recon_loss(x_est = R_est, x = self.R[mask_2,:][:, mask_r], mode = "RNA")
            
            loss = self.alpha[0] * loss_rna + self.alpha[1] * loss_atac
            loss.backward()
            self.optimizer.step()
                
            if (t+1) % self.interval == 0:
                print('Epoch {}, Validating Loss: {:.4f}'.format(t, loss.item()))
                info = [
                ...
                if loss.item() < best_loss:
                    best_loss = loss.item()
                    torch.save(self.state_dict(), f'../check_points/deep_{self.N}.pt')
                    count = 0
                else:
                    count += 1
                    if count % 20 == 0:
                        ...
                        if self.optimizer.param_groups[0]['lr'] < 1e-5:
                            ...
                            self.load_state_dict(torch.load(f'../check_points/deep_{self.N}.pt'))
                            count = 0
