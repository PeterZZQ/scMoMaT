...
class cfrm_diff2(Module):
    def __init__(self, counts, N1=3, N2 = 3, K = 3, batch_size=0.3, interval=10, lr=1e-3, alpha = [1000, 1000, 100, 100, 0.0, 0.0, 0.0], seed = None, learn_gact = False):
        ...
        # not always the same scale, only the latent dimension of K is shared
        self.N_feat = self.N1 + self.N2 - K
        self.A_g = Parameter(torch.rand((self.N_feat, min(self.N_feat * 2, self.N_feat + 5))))
        self.A_r = Parameter(torch.rand((self.N_feat, min(self.N_feat * 2, self.N_feat + 5))))

        self.C_1 = Parameter(torch.rand(self.G.shape[0], self.N_feat))
        self.C_2 = Parameter(torch.rand(self.R.shape[0], self.N_feat))

        
        self.C_r = Parameter(torch.rand(self.R.shape[1], min(self.N_feat * 2, self.N_feat + 5)))
        self.C_g = Parameter(torch.rand(self.G.shape[1], min(self.N_feat * 2, self.N_feat + 5)))
        ...
        self.mask_diagg = torch.eye(min(self.N_feat * 2, self.N_feat + 5)).to(device)
        self.mask_diagr = torch.eye(min(self.N_feat * 2, self.N_feat + 5)).to(device)
